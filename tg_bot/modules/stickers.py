...
def steal(bot: Bot, update: Update, args: List[str]):
    ...
    packnum = 0
    packname = str(user.id) + " _by_ "+bot.username
    packname_found = 0
    max_stickers = 120
    while packname_found == 0:
        try:
            ...
            if len(stickerset.stickers) >= max_stickers:
                    packnum += 1
                    packname = str(packnum) + " _ " + str(user.id) + " _by_ "+bot.username
            else:
                packname_found = 1
...
def makepack_internal(msg, user, png_sticker, emoji, bot, packname, packnum):
    ...
    try:
        ...
        success = bot.create_new_sticker_set(user.id, packname, f"{name}'s sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ" + extra_version,
