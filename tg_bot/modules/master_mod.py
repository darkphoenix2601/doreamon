...
def get_id(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id:
        ...
        if msg.reply_to_message and msg.reply_to_message.forward_from:
            ...
            msg.reply_text(f"➥ <b>User {html.escape(user.first_name)}'s </b>ㅤid is ☞ <code>{user.id}</code>.\nㅤ╚»{mention_html(user.id, '<b>User Link</b>')}\n➥ <b>Current Chat id</b> ☞ <code>{chat.id}</code>",
                           parse_mode=ParseMode.HTML)
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if disaster_level_present:
        text += ' [<a href="http://t.me/{}?start=disasters">𝙲𝙻𝙸𝙲𝙺 𝙷𝙴𝚁𝙴</a>]'.format(bot.username)
